...
# DeepGMM method to estimate non-parametric inverse demand
...
class DCM(AbstractMethod):
    def __init__(self, input_dim_g, input_dim_f, enable_cuda=False):
        ...
        g_models = [
            MLPModel(input_dim=self.input_dim_g, layer_widths=[64, 32, 16],
                     activation=nn.LeakyReLU).double(),
        ]
        f_models = [
            MLPModel(input_dim=self.input_dim_f, layer_widths=[64, 32, 16],
                     activation=nn.LeakyReLU).double(),
        ]

...
# DeepGMM method to estimate functions of simple/moderate complexity for boostIV MC
...
class simple_method(AbstractMethod):
    def __init__(self, input_dim_g, input_dim_f, enable_cuda=False):
        ...
        g_models = [
            MLPModel(input_dim=self.input_dim_g, layer_widths=[24, 12],
                     activation=nn.LeakyReLU).double(),
        ]
        f_models = [
            MLPModel(input_dim=self.input_dim_f, layer_widths=[24, 12],
                     activation=nn.LeakyReLU).double(),
        ]
